...
class TestUtils {
  void generateRandomStatistics(
    CounterRepository counterRepository,
    int folderId,
    DateTime selectedDate,
  ) async {
    List<Counter> counters = await counterRepository.getCountersByFolderId(
      folderId,
    );
    for (var counter in counters) {
      ...
      generateRandomStatisticsForMonth(
        counterRepository,
        counter.id!,
        selectedDate,
      );
  ...
  void generateRandomStatisticsForDay(
    CounterRepository counterRepository,
    int counterId,
    DateTime selectedDate,
    int nbStats,
    ...
    for (int i = 0; i < nbStats; i++) {
      ...
      await counterRepository.addStatistics(
        Statistics.fromJson({
          'counterId': counterId,
  ...
  void generateRandomStatisticsForWeek(
    CounterRepository counterRepository,
    int counterId,
    DateTime selectedDate,
  ) async {
    ...
    for (int i = 0; i < 7; i++) {
      generateRandomStatisticsForDay(
        counterRepository,
        counterId,
        firstDayOfWeek.add(Duration(days: i)),
        5,
      );
    }
  }

  void generateRandomStatisticsForMonth(
    CounterRepository counterRepository,
    int counterId,
    DateTime selectedDate,
  ) async {
    ...
    for (int i = 0; i < 4; i++) {
      generateRandomStatisticsForWeek(
        counterRepository,
        counterId,
        firstDayOfMonth.add(Duration(days: 7 * i)),
      );
    }
  }

  void generateRandomStatisticsForYear(
    CounterRepository counterRepository,
    int counterId,
    DateTime selectedDate,
  ) async {
